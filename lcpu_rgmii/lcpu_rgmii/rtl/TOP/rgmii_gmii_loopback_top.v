...
module rgmii_gmii_loopback_top #(
    parameter cpu_buf_addr_width = 11,  // 地址位宽
    parameter cpu_buf_data_width = 8,   // 数据位宽
    parameter cpu_buf_para_width = 3    // 参数位宽
) (
    // RGMII
  ...
  wire [7:0] dbg_fifo_wdata;
  wire       dbg_fifo_wen, dbg_fifo_push;

  // cpu_channel_reg 32-bit → 截断
  wire [31:0] reg_rd_raddr_32, reg_wr_waddr_32, reg_wr_wdata_32, reg_wr_wpkt_len_32;
  assign cpu_rd_raddr = reg_rd_raddr_32[cpu_buf_addr_width-1:0];
  assign cpu_wr_waddr = reg_wr_waddr_32[cpu_buf_addr_width-1:0];
  assign cpu_wr_wdata = reg_wr_wdata_32[cpu_buf_data_width-1:0];
  assign cpu_wr_wpkt_len = reg_wr_wpkt_len_32[cpu_buf_addr_width:0];
  ...
  cpu_channel #(
      ...
      .dbg_fifo_wdata   (dbg_fifo_wdata),
      .dbg_fifo_wen     (dbg_fifo_wen),
      .dbg_fifo_push    (dbg_fifo_push),
      ...
      .cpu_wr_wpkt_para({cpu_buf_para_width{1'b0}})
  );
  ...
  //============================================================================
  // 6. ILA — 全链路: RGMII→GMII → MAC RX → FIFO TX → GMII TX → PC
  //============================================================================
  ila_0 u_ila_0 (
      .clk    (clk_125m),
      .probe0 ({gmii_rx_dv, gmii_rx_er, gmii_rxd}),                            // 10bit
      .probe1 ({mac_rx_sop, mac_rx_en, mac_rx_eop, mac_rx_err, mac_rx_data}),  // 12bit
      .probe2 ({cpu_tx_sop, cpu_tx_en, cpu_tx_eop, cpu_tx_err, cpu_tx_data}),  // 12bit
      .probe3 ({gmii_tx_en, gmii_tx_er, gmii_txd}),                            // 10bit
      .probe4 ({dbg_fifo_wen, dbg_fifo_push, dbg_fifo_wdata}),                  // 10bit
      .probe5 ({cpu_rd_empty, pkt_drop_cnt})                                   // 9bit
  );
