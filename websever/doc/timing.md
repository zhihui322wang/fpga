...
## 第2段: MAC RX → FIFO (帧缓存 → 单字节过滤 → extract → package_fifo)

### 过滤原理

```
SOP → cnt 开始计数
cnt==61 且 data==0x77('w') → match=1
EOP 时 match=1 → 数据下传 (进 FIFO)
```
...
```wavedrom
{
  ...
  signal: [
    ...
    { name: "mac_rx_data",   wave: "x......==|=======x",
      data: ["D0","","0x77","D62","D63","FCS0","FCS1","FCS2","FCS3"] },
    { name: "mac_rx_eop",    wave: "0...............10" },
    { name: "mac_rx_err",    wave: "0...............10" },
    {},
    { name: "rx_byte_cnt",   wave: "x......====|....=x",
      data: ["1","2","3","","68"] }
  ]
...
│ RGMII→   │    │ 过滤→    │    │ JTAG→    │    │ JTAG→    │    │ CRC32→   │    │ GMII→    │
│ GMII     │    │ extract  │    │ 逐字节读  │    │ 逐字节写  │    │ FCS插入  │    │ RGMII    │
└──────────┘    └──────────┘    └──────────┘    └──────────┘    └──────────┘    └──────────┘
     ↑                                              │
     └────────────────── 环回通路 ──────────────────┘
```
