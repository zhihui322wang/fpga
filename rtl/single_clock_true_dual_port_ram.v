// sclk_tdpram.v — 同步真双口RAM（True Dual-Port RAM，同一时钟域）
//
// 两个端口（A/B）均支持独立读写，使用同一时钟。
// 读写模式：READ_FIRST（写操作期间输出写入前的旧数据，非阻塞赋值保证此时序）。
//
// 参数说明：
//   data_width      : 数据位宽（bit）
...
//
// 冲突行为：
//   双端口同时写入同一地址（address_a == address_b，wren_a && wren_b）时结果未定义，
//   用户应在逻辑层保证不出现此情况（与标准Block RAM行为一致）。

`include "define.sv"

module single_clock_true_dual_port_ram #(
    ...
    input  [(data_width/8)-1:0] wren_byte_b
);
  ...
  // 对齐到整数块边界的实际分配深度
  // 综合工具据此推断Block RAM数量，以1块为粒度增长
  // 示例：depth=8193, WORDS_PER_BRAM=1024 → NUM_BRAMS=9 → ALLOC_DEPTH=9216 → 9块
  localparam ALLOC_DEPTH = NUM_BRAMS * WORDS_PER_BRAM;

  // 内部地址线位宽：由ALLOC_DEPTH精确计算，而非由外部addr_bits决定
  // 这是关键：若用外部addr_bits=14，Vivado会认为深度是2^14=16384，推断16块
  // 改用ALLOC_ADDR_BITS=$clog2(9216)=14（但实际分配9216，工具识别9块）
  // 注意：$clog2必须在综合期间常量计算，Vivado/Quartus均支持
  ...
  localparam MEM_SIZE_BITS = ALLOC_DEPTH * data_width;
  ...
  // intel(Cyclone IV M9K) true dual-port 宽深比：
  //   宽度 > 9bit 时采用 512 深度 bank，可避免 13bit 地址被扩到 8192 深度后
  //   退化为 32 个 1-bit 宽 RAM 块的问题。
  //   例如 5120x32 -> 10 banks x (512x32) -> 20 x M9K（期望值）。
  ...
  // Default byte enables to all-1 when not connected
  wire [BYTE_EN_BITS-1:0] wren_byte_b_eff = wren_byte_b;
  ...
  generate
    // xilinx(Xilinx)原语：按单块BRAM粒度分段实例化，避免9Kx32被工具扩成16块
    if (vendor == "xilinx") begin : gen_xilinx_xpm_tdpram
      ...
      reg [BANK_SEL_BITS-1:0] bank_sel_a_r;
      reg [BANK_SEL_BITS-1:0] bank_sel_b_r;
      reg bank_sel_a_valid_r;
      reg bank_sel_b_valid_r;

      wire [data_width-1:0] q_a_bank[0:NUM_BRAMS-1];
      ...
      always @(posedge clk) begin
        bank_sel_a_r <= bank_sel_a;
        bank_sel_b_r <= bank_sel_b;
        bank_sel_a_valid_r <= addr_a_valid;
        bank_sel_b_valid_r <= addr_b_valid;
      end

      always_comb begin
        q_a_mux = {data_width{1'b0}};
        for (xa = 0; xa < NUM_BRAMS; xa = xa + 1) begin
          if (bank_sel_a_r == xa[BANK_SEL_BITS-1:0]) q_a_mux = q_a_bank[xa];
        end
      end
      ...
      assign q_a_int = bank_sel_a_valid_r ? q_a_mux : {data_width{1'b0}};
      assign q_b_int = bank_sel_b_valid_r ? q_b_mux : {data_width{1'b0}};

      genvar bi;
      for (bi = 0; bi < NUM_BRAMS; bi = bi + 1) begin : xpm_bank
        wire [BYTE_EN_BITS-1:0] wea_b = {BYTE_EN_BITS{wren_a && addr_a_valid && (bank_sel_a == bi[BANK_SEL_BITS-1:0])}};
        wire bank_b_active = wren_b && addr_b_valid && (bank_sel_b == bi[BANK_SEL_BITS-1:0]);
        wire [BYTE_EN_BITS-1:0] web_b = wren_byte_b_eff & {BYTE_EN_BITS{bank_b_active}};
      ...
      reg [INTEL_BANK_SEL_BITS-1:0] bank_sel_a_r;
      reg [INTEL_BANK_SEL_BITS-1:0] bank_sel_b_r;
      reg bank_sel_a_valid_r;
      reg bank_sel_b_valid_r;

      wire [data_width-1:0] q_a_bank[0:INTEL_NUM_BANKS-1];
      wire [data_width-1:0] q_b_bank[0:INTEL_NUM_BANKS-1];

      integer ia;
      integer ib;
      reg [data_width-1:0] q_a_mux_i;
      ...
      always_comb begin
        q_a_mux_i = {data_width{1'b0}};
        for (ia = 0; ia < INTEL_NUM_BANKS; ia = ia + 1) begin
          if (bank_sel_a_r == ia[INTEL_BANK_SEL_BITS-1:0]) q_a_mux_i = q_a_bank[ia];
        end
      end
      ...
      assign q_a_int = bank_sel_a_valid_r ? q_a_mux_i : {data_width{1'b0}};
      ...
      for (ii = 0; ii < INTEL_NUM_BANKS; ii = ii + 1) begin : altsyncram_bank
        ...
        wire wren_b_i = wren_b && addr_b_valid && (bank_sel_b == ii[INTEL_BANK_SEL_BITS-1:0]);
        ...
        altsyncram #(
            ...
            .ram_block_type("M9K"),
            // Use NORMAL to avoid Stratix-family sim model rejecting BYPASS at time 0.
      ...
      // 当 data_width 非 8 的整数倍时，剩余高位需单独写入，
      // 否则在仿真中会保持为 X（参照 single_clock_simple_dual_port_ram 修复）。
      ...
      // 当 data_width 非 8 的整数倍时，剩余高位需单独写入，
      // 否则在仿真中会保持为 X（参照 single_clock_simple_dual_port_ram 修复）。
      // 使用 generate-if 替代运行时 if，理由同端口A。
      if (REM_BITS > 0) begin : gen_port_b_nonbyte
        always @(posedge clk) begin
          if (wren_b) begin
            for (byte_idx_b = 0; byte_idx_b < BYTE_EN_BITS; byte_idx_b = byte_idx_b + 1) begin
              if (wren_byte_b_eff[byte_idx_b])
                ram[eff_addr_b][byte_idx_b*8+:8] <= data_b[byte_idx_b*8+:8];
            end
            // 写入非 8 整数倍宽度的剩余高位（当 data_width<8 时写入全部位）
            if (wren_byte_b_eff[0])
              ram[eff_addr_b][data_width-1-:REM_BITS] <= data_b[data_width-1-:REM_BITS];
          end
          q_b_r <= ram[eff_addr_b];
        end
      end else begin : gen_port_b_aligned
        always @(posedge clk) begin
          if (wren_b) begin
            for (byte_idx_b = 0; byte_idx_b < BYTE_EN_BITS; byte_idx_b = byte_idx_b + 1) begin
              if (wren_byte_b_eff[byte_idx_b])
...
// generate condition at line 80-90 due to cross-hierarchy parameter propagation.
...
module ERROR__addr_width_must_be_gte_alloc_addr_bits__single_clock_tdp ();
endmodule

// synthesis translate_off
`ifndef IVL_SIM
module tb_single_clock_true_dual_port_ram;
  ...
  initial begin
    $dumpfile("tb_single_clock_true_dual_port_ram.vcd");
    $dumpvars(0, tb_single_clock_true_dual_port_ram);
    wren_a = 0;
    wren_b = 0;
    data_a = 0;
    data_b = 0;
    ...
    @(posedge clk);
    #1;
    if (q_a !== 8'hAB) $display("FAIL: q_a=%h, expect AB", q_a);
    else $display("PASS: q_a=AB");
