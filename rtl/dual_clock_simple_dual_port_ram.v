// dclk_sdpram.v — 异步简单双口RAM（Simple Dual-Port RAM，独立时钟域）
//
// 端口A：只写（wren_a, data_a, address_a），时钟 clock_a
// 端口B：只读（address_b, q_b），时钟 clock_b
...
//
// Block RAM 粒度分配原理（与 single_clock_simple_dual_port_ram 一致）：
//   WORDS_PER_BRAM = block_ram_size × 1024 / data_width
//   NUM_BRAMS      = ceil(depth / WORDS_PER_BRAM)
//   ALLOC_DEPTH    = NUM_BRAMS × WORDS_PER_BRAM
//
// 综合属性兼容性：
...
module dual_clock_simple_dual_port_ram #(
  ...
  // ── Block RAM 粒度分配（与 single_clock_simple_dual_port_ram 一致）──
  // 每块Block RAM能存储的字数
  ...
  generate
    // xilinx(Xilinx)原语：xpm_memory_sdpram（Simple Dual-Port RAM，独立时钟）
    if (vendor == "xilinx") begin : gen_xilinx_xpm_sdpr
      ...
      // 读侧 bank 选择寄存（clock_b 时钟域，补偿 1 周期读延迟）
      ...
      // 端口B：读（clock_b 时钟域，READ_FIRST 模式）
      always @(posedge clock_b) begin
...
module ERROR__addr_width_must_be_gte_alloc_addr_bits__dual_clock_simple_dpram ();
endmodule
...
module tb_dual_clock_simple_dual_port_ram;
  ...
  localparam depth = 20;  // 非2的幂，验证粒度分配
  localparam addr_width = 5;  // $clog2(20) = 5
  ...
  initial begin
    $dumpfile("tb_dual_clock_simple_dual_port_ram.vcd");
    $dumpvars(0, tb_dual_clock_simple_dual_port_ram);
    wren_a = 0;
    data_a = 0;
    address_a = 0;
    address_b = 0;
    ...
    // 端口A（clock_a）写入地址0，值0xAB
    @(posedge clock_a);
    wren_a = 1;
    address_a = 5'h00;
    data_a = 8'hAB;
    @(posedge clock_a);
    // 端口A（clock_a）写入地址1，值0xCD
    address_a = 5'h01;
    data_a = 8'hCD;
    @(posedge clock_a);
    wren_a = 0;
    ...
    // 等待写完成
    repeat (3) @(posedge clock_b);

    // 端口B（clock_b）读地址0
    address_b = 5'h00;
    @(posedge clock_b);
    @(posedge clock_b);  // 1 cycle read latency
    #1;
