...
| 层次 | 文件 | 代码量 | 核心职责 |
|------|------|--------|----------|
| 应用层 | `main.c` | ~180 行 | 主循环帧分发、LED 流水灯 + 网络闪烁状态机 |
| 以太网层 | `eth.c` | ~65 行 | eth_rx_frame / eth_tx_frame，FIFO 读写封装 |
| ARP 层 | `arp.c` | ~100 行 | ARP Request 解析、Reply 原地构建、单条 MAC 缓存 |
| IP 层 | `ip.c` | ~120 行 | IPv4 头校验和验证、协议号分派、src/dst IP 交换 |
| ICMP 层 | `icmp.c` | ~75 行 | Echo Request 解析、Reply 零拷贝构建、ICMP 校验和重算 |
| 硬件抽象层 | `lcpu_general.h` | ~200 行 | 寄存器地址宏、FIFO 操作宏、rdcycle 定时器、协议常量 |

**合计约 740 行 C 代码**，无动态内存分配，所有缓冲区静态分配，栈空间安全可控。
...
`section(".text.bootloader")` + linker.ld 中 `KEEP` 确保该函数占据地址 0x00000000。不进行额外的栈初始化或 `.bss` 清零——picolibc 的 CRT 在 `main()` 之前已完成这些工作。
...
int main(void) {
    arp_init();                    // 清空 ARP 缓存
    上电自检 (LED 全亮 → 全灭);     // 视觉确认硬件就绪
    读取本机 MAC 到 my_mac[6];

    while (1) {
        led_flow_update();         // 非阻塞 LED 状态机 (每次循环执行)

        if (eth_rx_frame(rx_buffer, &len) == 0) {   // 有包
            eth_type = rx_buffer[12] << 8 | rx_buffer[13];

            if (eth_type == ETH_TYPE_ARP)
                arp_process(rx_buffer, len);         // ARP 处理
            else if (eth_type == ETH_TYPE_IP)
                ip_process(rx_buffer, len);          // IP 处理
        }
    }
}
```
...
**核心收获**:
1. 掌握了 RISC-V 裸机启动流程：reset_entry → main，理解 linker.ld 的内存布局
2. 理解了基于 rdcycle 的非阻塞状态机设计模式（无需中断）
3. 建立了"轮询 + 分派"的主循环架构（类似于事件驱动模型）
4. 掌握了单缓冲零拷贝的帧处理思想
...
5. 缓存发送方 IP (g_ip_sender, 供 ICMP 使用)
